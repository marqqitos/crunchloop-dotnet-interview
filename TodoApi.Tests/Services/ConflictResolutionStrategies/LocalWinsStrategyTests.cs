...
public class LocalWinsStrategyTests
{
    private readonly Mock<ILogger> _mockLogger;
    private readonly LocalWinsStrategy<TestLocalEntity, TestExternalEntity> _strategy;

    public LocalWinsStrategyTests()
    {
        _mockLogger = new Mock<ILogger>();
        _strategy = new LocalWinsStrategy<TestLocalEntity, TestExternalEntity>(_mockLogger.Object);
    }

    [Fact]
    public void StrategyType_ShouldReturnLocalWins()
    {
        // Act
        var result = _strategy.StrategyType;

        // Assert
        Assert.Equal(ConflictResolutionStrategy.LocalWins, result);
    }
    ...
    public void GetResolutionReason_ShouldReturnCorrectReason()
    {
        ...
        // Assert
        var expectedReason = $"Local changes take precedence. External changes made at {externalLastModified:yyyy-MM-dd HH:mm:ss} " +
                           $"will be ignored in favor of local changes made at {localLastModified:yyyy-MM-dd HH:mm:ss}.";
    ...
    public void GetResolutionReason_WithEmptyModifiedFields_ShouldHandleGracefully()
    {
        ...
        Assert.Contains("Local changes take precedence", result);
    }
    ...
    {
        // Arrange & Act
        var strategy = new LocalWinsStrategy<TestLocalEntity, TestExternalEntity>(_mockLogger.Object);

        // Assert
        Assert.NotNull(strategy);
        Assert.Equal(ConflictResolutionStrategy.LocalWins, strategy.StrategyType);
    }
